...
import { createAccount, fetchAdmin, isNoteUpdatedEventFired, isFired, ensureNoteEditMetadata, type LoginUser, type Request, resolveRemoteUser, sleep, createRole } from './utils.js';
...
describe('Timeline', () => {
    ...
    async function postAndCheckReception<C extends TimelineChannel>(
        ...
        const streamingFired = await isFired(
            ...
            async () => {
                note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text, ...noteParams })).createdNote);
            },
    ...
    describe('homeTimeline', () => {
        ...
        describe('Check reception of remote followee\'s Note', () => {
            ...
            test.failing('Receive remote followee\'s visible specified-only reply to invisible specified-only Note', async () => {
                const note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'a', visibility: 'specified' })).createdNote);
