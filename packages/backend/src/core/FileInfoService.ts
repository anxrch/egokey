...
export class FileInfoService {
     ...
     *
     * @param path ファイルパス
     * @returns ビデオトラックがあるかどうか（ffprobeが利用できない場合は音声のみとみなす）
     */
    @bindThis
        ...
        return new Promise((resolve) => {
            try {
                FFmpeg.ffprobe(path, (err, metadata) => {
                    if (err) {
                        const fallbackHasVideoTrack = !this.isProbeMissingError(err);
                        sublogger.warn(`Could not check the video file. Assuming ${fallbackHasVideoTrack ? 'video track exists' : 'audio-only'}. File path: ${path}`, err);
                        resolve(fallbackHasVideoTrack);
                        return;
                    }
                ...
                const fallbackHasVideoTrack = !this.isProbeMissingError(err);
                sublogger.warn(`Could not check the video file. Assuming ${fallbackHasVideoTrack ? 'video track exists' : 'audio-only'}. File path: ${path}`, err as Error);
                resolve(fallbackHasVideoTrack);
            }
        });
    }

    private isProbeMissingError(err: unknown): boolean {
        if (!(err instanceof Error)) return false;
        const message = err.message ?? '';
        if (message.toLowerCase().includes('ffprobe')) return true;
        if (message.toLowerCase().includes('ffmpeg')) return true;
