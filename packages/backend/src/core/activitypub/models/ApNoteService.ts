...
export class ApNoteService {
    ...
    public async updateNote(value: string | IObject, resolver?: Resolver, silent = false): Promise<boolean> {
        ...
        if (oldNote == null) {
            this.logger.warn(`Note update failed: note not found for URI ${uri}`);
            throw new Error('Note is not registed');
        }
        //#endregion
        ...
        // 投稿者が凍結されていたらスキップ
        if (actor.isSuspended) {
            this.logger.warn(`Note update failed: actor ${actor.uri} is suspended`);
            throw new Error('actor has been suspended');
        }

        // Validate that the actor is the note author
        if (oldNote.userId !== actor.id) {
            this.logger.warn(`Note update failed: actor ${actor.uri} is not the author of note ${oldNote.id}`);
            throw new Error('actor is not the note author');
        }
        ...
        // Guard against malicious attribute changes
        // Don't allow changing certain immutable fields
        ...
        // Only update fileIds if attachments are present in the update
        if (note.attachment !== undefined) {
            updateData.fileIds = files.map(f => f.id);
        }

        // Only update poll if present in the update (check if note is a Question)
        const noteAsQuestion = note as any;
        if (noteAsQuestion.oneOf !== undefined || noteAsQuestion.anyOf !== undefined) {
            updateData.poll = poll;
        }

        await this.noteUpdateService.update(actor, oldNote, updateData, silent);
