...
import { addCustomEmoji, createAccount, createModerator, deepStrictEqualWithExcludedFields, ensureNoteEditMetadata, type LoginUser, resolveRemoteNote, resolveRemoteUser, sleep, uploadFile } from './utils.js';
...
describe('Note', () => {
    ...
    describe('Note content', () => {
        test('Consistency of Public Note', async () => {
            ...
            const note = ensureNoteEditMetadata((await alice.client.request('notes/create', {
                text: 'I am Alice!',
                fileIds: [image.id],
                poll: {
                    choices: ['neko', 'inu'],
        ...
        test('Consistency of reply', async () => {
            const _replyedNote = ensureNoteEditMetadata((await alice.client.request('notes/create', {
                text: 'a',
            })).createdNote);
            const note = ensureNoteEditMetadata((await alice.client.request('notes/create', {
                text: 'b',
        ...
        test('Consistency of Renote', async () => {
            ...
            const renotedNote = ensureNoteEditMetadata((await alice.client.request('notes/create', {
                text: 'a',
            })).createdNote);
            const note = ensureNoteEditMetadata((await alice.client.request('notes/create', {
                text: 'b',
    ...
    describe('Other props', () => {
        test('localOnly', async () => {
            const note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'a', localOnly: true })).createdNote);
            rejects(
    ...
    describe('Deletion', () => {
        describe('Check Delete is delivered', () => {
            describe('To followers', () => {
                ...
                test('Check', async () => {
                    const note = ensureNoteEditMetadata((await bob.client.request('notes/create', { text: 'I\'m Bob.' })).createdNote);
        ...
        describe('Deletion of remote user\'s note for moderation', () => {
            ...
            test('Alice post is deleted in B', async () => {
                note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'Hello' })).createdNote);
    ...
    describe('Reaction', () => {
        describe('Consistency', () => {
            test('Unicode reaction', async () => {
                const note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'a' })).createdNote);
                const resolvedNote = await resolveRemoteNote('a.test', note.id, bob);
                const reaction = '😅';
        ...
        describe('Acceptance', () => {
            test('Even if likeOnly, remote users can react with custom emoji, but it is converted to like', async () => {
                const note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'a', reactionAcceptance: 'likeOnly' })).createdNote);
                const noteInB = await resolveRemoteNote('a.test', note.id, bob);
                const emoji = await addCustomEmoji('b.test');
            ...
            test('Even if nonSensitiveOnly, remote users can react with sensitive emoji, and it is not converted', async () => {
                const note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'a', reactionAcceptance: 'nonSensitiveOnly' })).createdNote);
                const noteInB = await resolveRemoteNote('a.test', note.id, bob);
    ...
    describe('Poll', () => {
        describe('Any remote user\'s vote is delivered to the author', () => {
            ...
            test('Bob creates poll and receives a vote from Carol', async () => {
                const note = ensureNoteEditMetadata((await bob.client.request('notes/create', { poll: { choices: ['inu', 'neko'] } })).createdNote);
                const noteInA = await resolveRemoteNote('b.test', note.id, carol);
