...
import { createAccount, deepStrictEqualWithExcludedFields, fetchAdmin, ensureNoteEditMetadata, type LoginUser, resolveRemoteNote, resolveRemoteUser, sleep } from './utils.js';
    ...
    describe('Profile', () => {
        ...
        describe('Pinning Notes', () => {
            ...
            test('Pinning followers-only Note is not delivered', async () => {
                const note = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'a', visibility: 'followers' })).createdNote);
                await alice.client.request('i/pin', { noteId: note.id });
                await sleep();
            ...
            test('Pinning normal Note is delivered', async () => {
                pinnedNote = ensureNoteEditMetadata((await alice.client.request('notes/create', { text: 'a' })).createdNote);
