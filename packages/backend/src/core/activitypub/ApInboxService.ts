...
export class ApInboxService {
    ...
    private async update(actor: MiRemoteUser, activity: IUpdate, resolver?: Resolver): Promise<string> {
        if (actor.uri !== activity.actor) {
            this.logger.warn(`Update activity rejected: actor mismatch. activity.actor=${activity.actor}, actor.uri=${actor.uri}`);
            return 'skip: invalid actor';
        }

        this.logger.debug(`Processing Update activity from ${actor.uri}`);
        ...
        this.logger.debug(`Update object type: ${objectType}`);

        if (isActor(object)) {
            await this.apPersonService.updatePerson(actor.uri, resolver, object);
            return 'ok: Person updated';
        } else if (objectType === 'Question') {
            await this.apQuestionService.updateQuestion(object, actor, resolver).catch(err => {
                this.logger.error(`Question update failed: ${err}`);
            });
            return 'ok: Question updated';
        } else if (objectType === 'Note') {
            try {
                await this.apNoteService.updateNote(object, resolver);
                this.logger.info(`Note updated successfully: ${getApId(object)}`);
                return 'ok: Note updated';
            } catch (err) {
                this.logger.error(`Note update failed: ${err}`);
                throw err;
            }
        } else {
            this.logger.warn(`Unknown Update type: ${objectType}`);
