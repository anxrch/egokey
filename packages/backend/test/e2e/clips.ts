...
import { api, ApiRequest, ensureNoteEditMetadata, failedApiCall, hiddenNote, post, signup, successfulApiCall } from '../utils.js';
...
describe('クリップ', () => {
    ...
    describe('に紐づくノート', () => {
        ...
        const notes = async (parameters: Misskey.entities.ClipsNotesRequest, request: Partial<ApiRequest<'clips/notes'>> = {}): Promise<Misskey.entities.Note[]> => {
            ...
            return result.map(note => ensureNoteEditMetadata(note));
        };
