...
export class ApRendererService {
	...
	public renderUpdate(object: string | IObject, user: { id: MiUser['id'] }): IUpdate {
		...
		// If object is a Note with to/cc, copy audiences from the note
		if (typeof object === 'object' && object.to) {
			update.to = object.to;
			update.cc = object.cc;
		} else {
			// Default to public for non-note updates (e.g., Person updates)
			update.to = ['https://www.w3.org/ns/activitystreams#Public'];
		}
