...
export const post = async (user: UserToken, params: misskey.Endpoints['notes/create']['req']): Promise<misskey.entities.Note> => {
    ...
    return ensureNoteEditMetadata((res.body ? res.body.createdNote : null)!);
};

type NoteWithOptionalEditMetadata = Omit<misskey.entities.Note, 'isEdited' | 'editCount' | 'latestEditedAt'> & Partial<Pick<misskey.entities.Note, 'isEdited' | 'editCount' | 'latestEditedAt'>>;

export const ensureNoteEditMetadata = (note: NoteWithOptionalEditMetadata): misskey.entities.Note => ({
    ...note,
    isEdited: note.isEdited ?? false,
    editCount: note.editCount ?? 0,
    latestEditedAt: note.latestEditedAt ?? null,
});
