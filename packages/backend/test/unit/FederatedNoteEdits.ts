...
describe('Federated Note Edits', () => {
	...
	beforeAll(async () => {
		...
		resolver = new MockResolver(await app.resolve(MockResolver));
	});
	...
	describe('Outbound Update activities', () => {
		test('should render Update activity with full note object', async () => {
			...
			expect(updateActivity.actor).toBe(`https://example.com/users/${localUser.id}`);
		...
		test('should respect followers-only visibility in Update activity', async () => {
			...
			expect(updateActivity.to).toEqual([`https://example.com/users/${localUser.id}/followers`]);
